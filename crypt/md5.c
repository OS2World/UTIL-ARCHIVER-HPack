...
/* The Mysterious Constants used in the MD5 transformation.  Note that some
   compilers (newer gcc's and Arm C) complain about the fact that some of
   these unsigned longs aren't followed by a 'U' (this behaviour is contrary
   to Section 3.1.3.2 which says the type is automatically made a long int).
   On the other hand some compilers choke if they are.  As of late 1993 it's
   assumed that if your compiler can't handle unsigned longs its broken, so
   it's now 'UL' rather than 'L' as it was before.  And there was much
   wailing, and gnashing of parsers... */
